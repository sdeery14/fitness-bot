...
class EvaluationHarness:
    ...
    async def run_single_test(self, test_case: TestCase, run_id: str) -> dict[str, Any]:
    ...
    async def run_evaluation(
        self, dataset: EvaluationDataset, run_name: str | None = None
    ) -> dict[str, Any]:
        ...
        with mlflow.start_run(run_name=run_name) as run:
            ...
            if successful_results:
                ...
                avg_latency = sum(
                    r["custom_metrics"]["latency"]["latency_ms"] for r in successful_results
                ) / len(successful_results)
                avg_cost = sum(
                    r["custom_metrics"]["cost"]["total_cost_usd"] for r in successful_results
                ) / len(successful_results)
                avg_tool_accuracy = sum(
                    r["custom_metrics"]["tool_usage"]["accuracy"] for r in successful_results
                ) / len(successful_results)
                ...
                judge_names = list(
                    successful_results[0]["judge_scores"]["individual_scores"].keys()
                )
...
async def main():
    ...
    async def mock_agent_runner(
        input_data: dict[str, Any], collector: MetricsCollector
    ) -> dict[str, Any]:
        ...
        collector.record_llm_call(
            model="gpt-4o", input_text=json.dumps(input_data), output_text="Generated workout phase"
        )

        # Record mock tool call
        collector.record_tool_call(
            tool_name="generate_workout_phase", parameters=input_data, is_correct=True
        )
    ...
    results = await harness.run_evaluation(
        dataset=workout_phase_dataset, run_name="baseline_evaluation"
    )
