...
from sqlalchemy import CheckConstraint, Date, DateTime, ForeignKey, Index, String, Text, Time, UniqueConstraint, text
...
class Schedule(Base, TimestampMixin):
    ...
    fitness_plan_id: Mapped[UUID] = mapped_column(PGUUID(as_uuid=True), ForeignKey("fitness_plans.id", ondelete="CASCADE"), nullable=False)
    
    # Schedule metadata
    start_date: Mapped[date] = mapped_column(Date, nullable=False)
    last_recalculated_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=text("CURRENT_TIMESTAMP"), nullable=False)
...
class ScheduleEntry(Base, TimestampMixin):
    ...
    entry_time: Mapped[time | None] = mapped_column(Time, nullable=True)  # Optional, for meal timing

    # Reference to actual workout or meal
    workout_id: Mapped[UUID | None] = mapped_column(PGUUID(as_uuid=True), ForeignKey("workouts.id", ondelete="SET NULL"), nullable=True)
    meal_id: Mapped[UUID | None] = mapped_column(PGUUID(as_uuid=True), ForeignKey("meals.id", ondelete="SET NULL"), nullable=True)

    # Completion tracking
    completion_status: Mapped[str] = mapped_column(String(50), nullable=False, server_default=text("'scheduled'"))  # scheduled, completed, skipped, rescheduled
